...
For example, models with high target-template sequence identity are expected to be the most likely to represent native metastable states, while lower sequence identity models would aid in sampling of more distant regions of accessible phase space.
...
For example, multiple sequence alignment algorithms~\cite{thompson:2011:msa-review}, allow alignments to be guided using sequence data from across the entire protein family of interest, while (multiple) structural alignment algorithms such as Modeller's salign routine~\cite{fiser:prot-sci:2000:modeller,sali:jmb:1993:modeller}, PROMALS3D~\cite{pei:2008:promals3d}, and Expresso and 3DCoffee~\cite{expresso,3dcoffee}, can additionally exploit structural data.
...
These ranged from -14180 kT to -3590 kT, with a median of -9533 kT, mean of -9564 kT, and a standard deviation of 1058 kT.
...
\shellcmd{conda install ensembler}\\
