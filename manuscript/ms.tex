...
The latest release can be installed \emph{via} the {\tt conda} package manager, and the latest source can be downloaded from \url{https://github.com/choderalab/ensembler}.
...
{\bf Ensembler} is written in Python, and can be used \emph{via} a command-line tool ({\tt ensembler}) or \emph{via} a flexible Python API to allow integration of its components into other applications.
...
\begin{figure*}[tb]
  ...
  Note that \emph{CPU-h} denotes the number of hours consumed by the equivalent of a single CPU hyperthread and \emph{GPU-h} on a single GPU---parallel execution \emph{via} MPI reduces wall clock time nearly linearly.
  }
...
The {\tt ensembler} command-line tool allows targets to be selected from UniProt---a freely accessible resource for protein sequence and functional data (\href{http://www.uniprot.org/}{uniprot.org})~\cite{uniprot:2015}---\emph{via} a UniProt search query.
...
Specific PDB chain IDs can optionally also be selected \emph{via} the {\tt -{}-chainids} flag.
...
Unresolved template residues can optionally be modeled into template structures with the {\tt loopmodel} subcommand, which employs a kinematic closure algorithm provided \emph{via} the {\tt loopmodel} tool of the Rosetta software suite~\cite{qian:nature:2007:modeller,wang:jmb:2007:modeller}.
...
The resultant models are available as part of a supplementary dataset which can be accessed \emph{via} the digital object identifier (DOI) \href{https://dx.doi.org/10.5061/dryad.7fg32}{10.5061/dryad.7fg32}.
...
The latest release can be installed \emph{via} the {\tt conda} package manager for Python (\href{http://conda.pydata.org}{conda.pydata.org}), using the two commands shown in Box~\ref{box:installation}.
